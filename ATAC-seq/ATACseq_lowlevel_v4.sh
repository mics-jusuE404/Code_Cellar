...
## Bam2Fq if uBAM:
if [[ $INPUT == "uBAM" ]]; then
  ls *_ubam.bam | parallel -j 16 "Bam2Fq {}"
  fi
...
ls *_cutsites.bed.gz | awk -F "_dedup.bed.gz" '{print $1}' | \
  parallel "$MACS callpeak -t {}_cutsites.bed.gz -n {} -g $GFLAG --extsize 100 --shift -50 --nomodel --keep-dup=all -f BED"
