...
## Assuming a duplicate of ATAC-seq experiments, call peaks on all datasets present in $(pwd),
## assuming that for a given cell type, the full BAM and one with only the read pairs with isize < 100bp are present:
## *_rep1/2_sorted.bam and *_rep1/2_sorted_isize100.bam
...
ls *.bam | awk -F ".bam" '{print $1}' | \
  parallel "macs2 callpeak -t {}.bam -n {} -g hs --nomodel -f BAMPE -q 0.01 --keep-dup=all --tempdir ./"
exit 

## Intersect full dataset:
ls *rep*_peaks.narrowPeak | grep -v 'isize100' | awk -F "_rep" '{print $1}' | sort -k1,1 -u | \
  parallel "bedtools intersect -a {}_rep1_sorted_peaks.narrowPeak -b {}_rep2_sorted_peaks.narrowPeak | \
...
ls *_sorted_isize100_peaks.narrowPeak | awk -F "_rep" '{print $1}' | sort -k1,1 -u | \
