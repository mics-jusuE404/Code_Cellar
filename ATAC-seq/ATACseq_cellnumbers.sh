...
function Fq2BamSE {
  ...
  samtools fastq -@ 4 ${BASENAME}_ubam.bam | \
  cutadapt -j 4 -a $ADAPTER1 -m 18 --max-n 0.1  | \
...
## fastqc:
ls *_ubam.bam | parallel -j 70 "fastqc -t 1 {}"
...
if [[ $MODE == "SE" ]]; then
  ls *_ubam.bam | awk -F "_ubam.bam" '{print $1}' | parallel -j 4 "Fq2BamSE {} $GENOME 2>> {}.log"
  fi
