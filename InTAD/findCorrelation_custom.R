...
findCorrelation_custom <- function(object,               ## InTADSig object
    ...
    ## but more efficiently using mclapply instead of for loop 
    ## and rbindlist instead of do.call(rbind, lists.of.dataframes)
    AfterCor <- function(allRes=allRes){
      
      allRes <- mclapply(allRes, function(x) na.omit(x), mc.cores=detectCores()/2)
      
      return(
        data.table::rbindlist(
          lapply(seq_len(length(allRes)), function(i){
