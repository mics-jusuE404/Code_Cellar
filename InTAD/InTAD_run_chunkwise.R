## Customized (and in parts ugly) InTAD wrapper script intended to be run on a 192GB node
## It assumes all required files (scripts, Rdata) to be in the same directory.

## Check for packages:
packageS <- c("InTAD", "data.table", "parallel")
...
## (=fpkm-normalized RNA-seq and ATAC-seq data)
...
## stop if loading of Rdata was unsuccessful:
...
InTAD_wrapper <- function(counts.atac,                 ## fpkm-norm. ATAC-seq counts
  ...
  ## Filter the RNA-seq count matrix against non-expressed genes based on ExprThresh.
  ## We do this here explicitely as the cutoff has been determined before using basically
  ## the filterGeneExpr.R script of InTAD which again relies on mclust.
  ## We used the log2(fpkm+1) count matrix where fpkm was calculated with DESeq2::fpkm(counts_raw, robust = TRUE),
  ## so corrected for library composition by DESeq2 scaling factors.
  
  message(paste("[Info]: Filtering RNA-seq count matrix for cutoff =", ExprThresh))
  
  ## log-transform data as this transformation was used to obtain the cutoff:
    ...
    unique(
      unlist(
        lapply(tmp.unique.celltype, function(U){
          ...
          return(as.integer(which(rowMeans(counts.rna[,tmp.which]) >= ExprThresh)))
          }
        )
      )
    )
  ...
  if (!FilterByGroupAverage){
    keep.rna <- which(as.numeric(apply(counts.rna, 1, function(x) max(x))) >= ExprThresh)
  }
  ...
  trash.can <- lapply(seq(1,permut.cycles), function(Q){
    ...
    ## It will split the cor analysis into chunks of 1000 signals at a time in a lapply-based function
    ## and spill the result to disk folder "./InTAD_raw",
    ## using 16 workers for the correlation analysis itself (hardcoded for now):
...
Study <- CLargs[1]
Permuts <- CLargs[2]
if(is.na(Study)) stop("Something wrong with study name read from command line!")
