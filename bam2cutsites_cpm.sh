...
bedtools bamtobed -bedpe -i <(samtools view -b -@ 8 -L primary.bed ${1%.bam_nsort.bam}) | mawk 'OFS="\t" { if ($9=="+") {print $1, $2+4, $2+5 "\n" $1, $6+3, $6+4} else if ($9=="-") {print $1, $2-5, $2-4 "\n" $1, $6-6, $6-5}}' | \
  sort -k1,1 -k2,2n --parallel=32 | \
  bedtools genomecov -bg -scale $SCALE -i - -g /scratch/tmp/a_toen03/Genomes/mm10/mm10_chromSizes.txt > ${1%.bam}_cutSites.bedGraph.bg

bedGraphToBigWig ${1%.bam}_cutSites.bedGraph.bg /scratch/tmp/a_toen03/Genomes/mm10/mm10_chromSizes.txt ${1%.bam}_cutSites_cpm.bigwig
