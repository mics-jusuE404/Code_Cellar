...
if [[ ${ASSAY} != "ATACseq" ]]; then rm sizeFactors.txt; fi
...
if [[ ${ASSAY} == "ChIPseq" ]]; then
  ...
  ls *_dedup.bam | parallel $JOBS "Bigwig_CPM {} ${ASSAY}"
...
if [[ ${ASSAY} == "ChIPseq" ]]; then
  ...
  | parallel -j 4 "FRiP_ChIP {}_noControl_peaks.narrowPeak {}_dedup.bam ${MODE} 2>> {}.log" > FRiP_scores.txt
